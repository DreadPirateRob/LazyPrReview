...
**Rendering (built-in, default):** lazypr implements its own diff renderer: line-number gutter (old/new), green/red coloring for added/deleted lines, styled hunk and file headers. Chroma syntax highlighting per file (language detected from extension) — *planned, not yet implemented*. Files with >5000 changed lines render without highlighting (performance floor). Viewport virtualization: only the visible region is styled per frame.
