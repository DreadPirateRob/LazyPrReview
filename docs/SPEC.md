...
- **Main side-by-side** (`|` toggles): a **read-only** view of one file with the old revision left and the new right. It is a **sticky preference, not a property of the content**: it survives focus changes, `[`/`]` file switches and the Files panel following its cursor, so browsing the PR stays side-by-side until you toggle it off. Every file-browsing render routes through one decision point (`renderDiffInMain`); thread positioning deliberately does not, because a thread has no side-by-side representation — a jump lands in the unified diff **without** clearing the preference, and `|` puts side-by-side back. The toggle acts on what is on screen rather than on the stored preference, so a press always changes the frame instead of silently flipping a flag. It pairs with `MainFileIndex = -1` — the same contract commit-scoped and directory-aggregate diffs use — so every line-scoped action (`c`, `v` range, `enter` on a thread, `space`) is inert while it is showing, and the unified render restores all of them. Inline thread blocks are deliberately absent: `refreshMainDiff` bails on that sentinel, so folding and thread focus would only half-work; threads stay in the unified view and panel `[4]`. A pane narrower than 80 columns renders unified silently on ordinary file changes (a toast on every render would be noise) and reports the reason on an explicit toggle; `gui.diffPager` refuses outright, since the pager owns its own layout.
