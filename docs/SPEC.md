...
- **Main side-by-side** (`|` from the unified diff, `|` again to leave): a **read-only** view of one file with the old revision left and the new right. It pairs with `MainFileIndex = -1` — the same contract commit-scoped and directory-aggregate diffs use — so every line-scoped action (`c`, `v` range, `enter` on a thread, `space`) is inert while it is open, and leaving restores the unified render of the same file along with all of them. Inline thread blocks are deliberately absent: `refreshMainDiff` bails on that sentinel, so folding and thread focus would only half-work; threads stay in the unified view and panel `[4]`. Refused with a toast when Main is narrower than 80 columns (try `+` for a wider mode), when Main is not showing a single file, or when `gui.diffPager` is set — the pager owns its own layout.
